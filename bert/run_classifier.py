...
class OpenTaskProcessor(DataProcessor):
    ...
    def __init__(self):
        self.labels = ['description', 'expected', 'others', 'reproduce']
    ...
    def get_dev_examples(self, data_dir):
        lines_x, lines_y = self._read_txt_(data_dir, 'valid_x.txt', 'valid_y.txt')
        examples = []
        for (i, line) in enumerate(zip(lines_x, lines_y)):
            guid = 'train-%d' % i
            text_a = tokenization.convert_to_unicode(line[0])
...
class IntensionMiningProcessor(DataProcessor):

    def __init__(self):
        self.labels = ['aspect evaluation', 'feature request', 'information giving', 'information seeking', 'problem discovery', 'solution proposal', 'others']
...
def main(_):
  ...
  processors = {
      ...
      "intensionmining": IntensionMiningProcessor
  }
