...
def compute_metrics(hypothesis, references, return_metrics=False, no_overlap=False):
    ...
    totalF, averageTotalF, totalPrec, totalRec = computeChrF(references[0], hypothesis, 2, 6, 2, None)
    ...
    if return_metrics:
        return ret_scores, scores["rouge-1"]["f"], scores["rouge-2"]["f"], scores["rouge-l"]["f"], averageTotalF
    else:
        return ret_scores

def main(test_msg_file :str, pre_msg_file :str):
    """Run NNGen with default given dataset using default setting"""
    compute_metrics(pre_msg_file, [test_msg_file])
