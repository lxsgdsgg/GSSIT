...
def context_prepare_pretrained_embeddings(opt, fields):
    if all([opt.both_embeddings is None,
            opt.src_embeddings is None,
            opt.tgt_embeddings is None]):
        return
    ...
    for side in ['src', 'tgt']:
        try:
            ...
            vocab = fields[side].vocab
        vocs.append(vocab)
    src_vocab, tgt_vocab = vocs
    des_vocab = rep_vocab = exp_vocab = oth_vocab = src_vocab

    skip_lines = 1 if opt.embeddings_type == "word2vec" else 0
    if opt.both_embeddings is not None:
        set_of_src_and_tgt_vocab = \
            set(src_vocab.stoi.keys()) | set(tgt_vocab.stoi.keys())
        logger.info("Reading encoder and decoder embeddings from {}".format(
            opt.both_embeddings))
        src_vectors, total_vec_count = \
            read_embeddings(opt.both_embeddings, skip_lines,
                            set_of_src_and_tgt_vocab)
        des_vectors = rep_vectors = exp_vectors = oth_vectors = tgt_vectors = src_vectors
        logger.info("\tFound {} total vectors in file".format(total_vec_count))
    else:
        if opt.src_embeddings is not None:
            logger.info("Reading encoder embeddings from {}".format(
    ...
    logger.info("After filtering to vectors in vocab:")
    if opt.src_embeddings is not None or opt.both_embeddings is not None:
        logger.info("\t* src: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(src_vocab, src_vectors))
        logger.info("\t* des: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(des_vocab, des_vectors))
        logger.info("\t* rep: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(rep_vocab, rep_vectors))
        logger.info("\t* exp: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(exp_vocab, exp_vectors))
        logger.info("\t* oth: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(oth_vocab, oth_vectors))
    if opt.tgt_embeddings is not None or opt.both_embeddings is not None:
        logger.info("\t* tgt: %d match, %d missing, (%.2f%%)"
                    % calc_vocab_load_stats(tgt_vocab, tgt_vectors))
    ...
    if opt.src_embeddings is not None or opt.both_embeddings is not None:
        logger.info("\nSaving encoder embeddings as:\n\t* enc: %s"
                    % enc_output_file)
        torch.save(
            convert_to_torch_tensor(src_vectors, src_vocab),
            enc_output_file
        )
        # set the opt in place
        opt.pre_word_vecs_enc = opt.pre_word_vecs_des = opt.pre_word_vecs_rep = \
            opt.pre_word_vecs_exp = opt.pre_word_vecs_oth = enc_output_file
    if opt.tgt_embeddings is not None or opt.both_embeddings is not None:
        logger.info("\nSaving decoder embeddings as:\n\t* dec: %s"
                    % dec_output_file)
        torch.save(
            convert_to_torch_tensor(tgt_vectors, tgt_vocab),
            dec_output_file
        )
        # set the opt in place
        opt.pre_word_vecs_dec = dec_output_file