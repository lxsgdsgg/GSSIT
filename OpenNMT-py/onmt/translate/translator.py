...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    elif model_opt.model_task == ModelTask.CONTEXT:
        translator = Context_Translator.from_opt(
            model,
            fields,
            opt,
            model_opt,
...
class Context_Translator(object):
    def __init__(
        self,
        model,
        fields,
        ...
        self.src_reader = src_reader
        self.des_reader = des_reader
        self.rep_reader = rep_reader
        self.exp_reader = exp_reader
        self.oth_reader = oth_reader
        self.tgt_reader = tgt_reader
        self.replace_unk = replace_unk
        if self.replace_unk and not self.model.decoder.attentional:
            raise ValueError("replace_unk requires an attentional decoder.")
    ...
    def from_opt(
        ...
        src_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        des_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        rep_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        exp_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        oth_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        tgt_reader = inputters.str2reader["text"].from_opt(opt)
        return cls(
            model,
            fields,
    ...
    def translate(
        ...
        des_data = {
            "reader": self.des_reader,
            "data": des,
            "features": src_feats
        }
        rep_data = {
            "reader": self.rep_reader,
            "data": rep,
            "features": src_feats
        }
        exp_data = {
            "reader": self.exp_reader,
            "data": exp,
            "features": src_feats
        }
        oth_data = {
            "reader": self.oth_reader,
            "data": oth,
            "features": src_feats
        }
        tgt_data = {
        ...
        _readers, _data = inputters.Dataset.config(
            [("src", src_data), ("des", des_data), ("rep", rep_data), ("exp", exp_data), ("oth", oth_data), ("tgt", tgt_data)]
        )
    ...
    @classmethod
    def validate_task(cls, task):
        if task != ModelTask.CONTEXT:
            raise ValueError(
                f"Translator does not support task {task}."
                f" Tasks supported: {ModelTask.CONTEXT}"
            )
    ...
    def _run_encoder(self, batch):
        ...
        des, des_lengths = (
            batch.des if isinstance(batch.des, tuple) else (batch.des, None)
        )
        rep, rep_lengths = (
            batch.rep if isinstance(batch.rep, tuple) else (batch.rep, None)
        )
        exp, exp_lengths = (
            batch.exp if isinstance(batch.exp, tuple) else (batch.exp, None)
        )
        oth, oth_lengths = (
            batch.oth if isinstance(batch.oth, tuple) else (batch.oth, None)
        )

        src_states, memory_bank, src_lengths = self.model.src_encoder(
            src, src_lengths
        )
        des_state, des_memory_bank, des_lengths = self.model.des_encoder(des, des_lengths)
        rep_state, rep_memory_bank, rep_lengths = self.model.rep_encoder(rep, rep_lengths)
        exp_state, exp_memory_bank, exp_lengths = self.model.exp_encoder(exp, exp_lengths)
        oth_state, oth_memory_bank, oth_lengths = self.model.oth_encoder(oth, oth_lengths)
        ...
        return src, src_states, memory_bank, src_lengths
