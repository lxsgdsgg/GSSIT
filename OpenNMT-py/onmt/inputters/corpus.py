...
class DatasetAdapter(object):
    ...
    valid_field_name = (
        'src', 'des', 'rep', 'exp', 'oth', 'tgt', 'indices', 'src_map', 'src_ex_vocab', 'alignment',
        'align')

    def __init__(self, fields, is_train, is_context):
    ...
    @staticmethod
    def _process(item, is_train, is_context=False):
        ...
        if is_context:
            maybe_example['des'] = {"des": ' '.join(maybe_example['des'])}
            maybe_example['rep'] = {"rep": ' '.join(maybe_example['rep'])}
            maybe_example['exp'] = {"exp": ' '.join(maybe_example['exp'])}
            maybe_example['oth'] = {"oth": ' '.join(maybe_example['oth'])}
    ...
    def _maybe_add_dynamic_dict(self, example, fields):
        ...
        if 'src_map' in fields and 'alignment' in fields:
            ...
            #         fields['des'].base_field,
            #         fields['rep'].base_field,
            #         fields['exp'].base_field,
            #         fields['oth'].base_field,
    ...
    def _to_examples(self, bucket, is_train=False):
        examples = []
        for item in bucket:
            maybe_example = self._process(item, is_train=is_train, is_context=self.is_context)
...
class ContextCorpus(object):
    def __init__(self, name, src, des, rep, exp, oth, tgt, align=None, src_feats=None):
        self.id = name
        self.src = src
        self.des = des
        self.rep = rep
    ...
    def load(self, offset=0, stride=1):
        if self.src_feats:
            ...
            features_files = []
        with exfile_open(self.src, mode='rb') as fs, \
                exfile_open(self.des, mode='rb') as fd, \
                exfile_open(self.rep, mode='rb') as fr, \
                exfile_open(self.exp, mode='rb') as fe, \
                exfile_open(self.oth, mode='rb') as fo, \
                exfile_open(self.tgt, mode='rb') as ft,\
                exfile_open(self.align, mode='rb') as fa:
            for i, (sline, dline, rline, eling, oline, tline, align, *features) in \
                    enumerate(zip(fs, fd, fr, fe, fo, ft, fa, *features_files)):
                if (i % stride) == offset:
                    sline = sline.decode('utf-8')
                    dline = dline.decode('utf-8')
                    rline = rline.decode('utf-8')
                    eling = eling.decode('utf-8')
    ...
    def __str__(self):
        cls_name = type(self).__name__
        return '{}({}, {}, {}, {}, {}, {}, align={}, src_feats={})'.format(
            cls_name, self.src, self.des, self.rep, self.exp, self.oth, self.tgt, self.align, self.src_feats)
        ...
        if is_train:
            for corpus_id, corpus_dict in opts.data.items():
                if corpus_id != CorpusName.VALID:
                    corpora_dict[corpus_id] = ContextCorpus(
                        corpus_id,
                        corpus_dict["path_src"],
                        corpus_dict["path_des"],
                        corpus_dict["path_rep"],
                        corpus_dict["path_exp"],
                        corpus_dict["path_oth"],
                        corpus_dict["path_tgt"],
                        corpus_dict["path_align"],
                        corpus_dict["src_feats"])
        else:
            if CorpusName.VALID in opts.data.keys():
                corpora_dict[CorpusName.VALID] = ContextCorpus(
                    CorpusName.VALID,
                    opts.data[CorpusName.VALID]["path_src"],
                    opts.data[CorpusName.VALID]["path_des"],
                    opts.data[CorpusName.VALID]["path_rep"],
                    opts.data[CorpusName.VALID]["path_exp"],
                    opts.data[CorpusName.VALID]["path_oth"],
...
class ParallelCorpusIterator(object):
    ...
    def __init__(self, corpus, transform,
                 skip_empty_level='warning', stride=1, offset=0, is_context=False):
        self.cid = corpus.id
        self.corpus = corpus
    ...
    def _tokenize(self, stream):
        for example in stream:
            ...
            if self.is_context:
                example['des'] = example['des'].strip('\n').split()
                example['rep'] = example['rep'].strip('\n').split()
                example['exp'] = example['exp'].strip('\n').split()
                example['oth'] = example['oth'].strip('\n').split()

                example['des_original'] = example['des_original'].strip('\n').split()
                example['rep_original'] = example['rep_original'].strip('\n').split()
                example['exp_original'] = example['exp_original'].strip('\n').split()
                example['oth_original'] = example['oth_original'].strip('\n').split()
...
class ContextCorpusIterator(object):
    def __init__(self, corpus, transform,
    ...
    def _tokenize(self, stream):
        for example in stream:
            ...
            example['src_original'] = \
                example['src_original'].strip("\n").split()
            example['des_original'] = \
                example['des_original'].strip("\n").split()
            example['rep_original'] = \
                example['rep_original'].strip("\n").split()
            example['exp_original'] = \
                example['exp_original'].strip("\n").split()
            example['oth_original'] = \
                example['oth_original'].strip("\n").split()
            example['tgt_original'] = \
                example['tgt_original'].strip("\n").split()
            if 'align' in example:
                example['align'] = example['align'].strip('\n').split()
...
def build_corpora_iters(corpora, transforms, corpora_info,
    ...
    for c_id, corpus in corpora.items():
        ...
        corpus_iter = ParallelCorpusIterator(
            ...
            skip_empty_level=skip_empty_level, stride=stride,
            offset=offset, is_context=is_context)
