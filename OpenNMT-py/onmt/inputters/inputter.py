...
def get_task_spec_tokens(data_task, pad, bos, eos):
    ...
    elif data_task == ModelTask.CONTEXT:
        return {
            "src": {"pad": pad, "bos": None, "eos": None},
            "des": {"pad": pad, "bos": None, "eos": None},
            "rep": {"pad": pad, "bos": None, "eos": None},
            "exp": {"pad": pad, "bos": None, "eos": None},
            "oth": {"pad": pad, "bos": None, "eos": None},
            "tgt": {"pad": pad, "bos": bos, "eos": eos},
        }
...
def get_fields(
    ...
    if is_context:
        des_field_kwargs = {
            "feats": src_feats,
            "include_lengths": True,
            "pad": task_spec_tokens["des"]["pad"],
            "bos": task_spec_tokens["des"]["bos"],
            "eos": task_spec_tokens["des"]["eos"],
            "truncate": src_truncate,
            "base_name": "des",
        }
        fields["des"] = fields_getters[src_data_type](**des_field_kwargs)

        rep_field_kwargs = {
            "feats": src_feats,
            "include_lengths": True,
            "pad": task_spec_tokens["rep"]["pad"],
            "bos": task_spec_tokens["rep"]["bos"],
            "eos": task_spec_tokens["rep"]["eos"],
            "truncate": src_truncate,
            "base_name": "rep",
        }
        fields["rep"] = fields_getters[src_data_type](**rep_field_kwargs)

        exp_field_kwargs = {
            "feats": src_feats,
            "include_lengths": True,
            "pad": task_spec_tokens["exp"]["pad"],
            "bos": task_spec_tokens["exp"]["bos"],
            "eos": task_spec_tokens["exp"]["eos"],
            "truncate": src_truncate,
            "base_name": "exp",
        }
        fields["exp"] = fields_getters[src_data_type](**exp_field_kwargs)

        oth_field_kwargs = {
            "feats": src_feats,
            "include_lengths": True,
            "pad": task_spec_tokens["oth"]["pad"],
            "bos": task_spec_tokens["oth"]["bos"],
            "eos": task_spec_tokens["oth"]["eos"],
            "truncate": src_truncate,
            "base_name": "oth",
        }
        fields["oth"] = fields_getters[src_data_type](**oth_field_kwargs)
    ...
    def batch_to_device(batch, device_id):
        ...
        if curr_device != device:
            if isinstance(batch.src, tuple):
                ...
                if hasattr(batch, 'des'):
                    batch.des = tuple([_.to(device) for _ in batch.des])
                    batch.rep = tuple([_.to(device) for _ in batch.rep])
                    batch.exp = tuple([_.to(device) for _ in batch.exp])
                    batch.oth = tuple([_.to(device) for _ in batch.oth])
            else:
                batch.src = batch.src.to(device)
                if hasattr(batch, 'des'):
                    batch.des = batch.des.to(device)
                    batch.rep = batch.rep.to(device)
                    batch.exp = batch.exp.to(device)
                    batch.oth = batch.oth.to(device)
...
def _build_fields_vocab(fields, counters, data_type, share_vocab,
    ...
    if is_context:
        build_fv_kwargs["des"] = build_fv_kwargs["rep"] = build_fv_kwargs["exp"] =\
            build_fv_kwargs["oth"] = build_fv_kwargs["src"]
    ...
    if data_type == 'text':
        ...
        _build_fv_from_multifield(
            ...
            size_multiple=vocab_size_multiple if not share_vocab else 1)
        if is_context:
            des_multifield = fields["des"]
            _build_fv_from_multifield(
                des_multifield,
                counters,
                build_fv_kwargs,
                size_multiple=vocab_size_multiple if not share_vocab else 1)
            rep_multifield = fields["rep"]
            _build_fv_from_multifield(
                rep_multifield,
                counters,
                build_fv_kwargs,
                size_multiple=vocab_size_multiple if not share_vocab else 1)
            exp_multifield = fields["exp"]
            _build_fv_from_multifield(
                exp_multifield,
                counters,
                build_fv_kwargs,
                size_multiple=vocab_size_multiple if not share_vocab else 1)
            oth_multifield = fields["oth"]
            _build_fv_from_multifield(
                oth_multifield,
                counters,
                build_fv_kwargs,
        ...
        if share_vocab:
            ...
            if is_context:
                des_field = des_multifield.base_field
                rep_field = rep_multifield.base_field
                exp_field = exp_multifield.base_field
                oth_field = oth_multifield.base_field
                _merge_context_field_vocabs(src_field, des_field, rep_field,
                    exp_field, oth_field, tgt_field,
...
def _merge_context_field_vocabs(src_field, des_field, rep_field,
                    exp_field, oth_field, tgt_field, vocab_size, min_freq,
                        vocab_size_multiple, specials):
    # in the long run, shouldn't it be possible to do this by calling
    # build_vocab with both the src and tgt data?
    init_specials = [tgt_field.unk_token, tgt_field.pad_token,
    ...
    all_specials = list(OrderedDict.fromkeys(
        ...
        if tok is not None))
    merged = sum(
        [src_field.vocab.freqs, des_field.vocab.freqs, rep_field.vocab.freqs,
         exp_field.vocab.freqs, oth_field.vocab.freqs, tgt_field.vocab.freqs], Counter()
    )
    merged_vocab = Vocab(
        merged, specials=all_specials,
        max_size=vocab_size, min_freq=min_freq
    ...
    if vocab_size_multiple > 1:
        _pad_vocab_to_multiple(merged_vocab, vocab_size_multiple)
    src_field.vocab = des_field.vocab = rep_field.vocab = exp_field.vocab = oth_field.vocab = merged_vocab
    tgt_field.vocab = merged_vocab
    assert len(src_field.vocab) == len(tgt_field.vocab) == len(des_field.vocab)
