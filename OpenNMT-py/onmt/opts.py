...
def config_opts(parser):
    ...
    group.add("-context", "--context", default=False, action="store_true",
              help="input context sentences")
...
def model_opts(parser):
    ...
    group.add(
        ...
        choices=[ModelTask.SEQ2SEQ, ModelTask.LANGUAGE_MODEL, ModelTask.CONTEXT],
    ...
    group.add('--enc_rnn_size', '-enc_rnn_size', type=int, default=400,
              help="Size of encoder rnn hidden states.")
    group.add('--context_size', '-context_size', type=int, default=100,
              help="Size of context encoder hidden states.")
    group.add('--dec_rnn_size', '-dec_rnn_size', type=int, default=400,
    ...
    # The proportion of context encoder in tasks
    group.add('--context_ratio', '-context_ratio', type=float, default=None,
              help="The proportion of context encoder in tasks.")
...
def _add_train_general_opts(parser):
    ...
    group.add('--pre_word_vecs_des', '-pre_word_vecs_des',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the context_encoder side. "
                   "See README for specific formatting instructions.")
    group.add('--pre_word_vecs_rep', '-pre_word_vecs_rep',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the context_encoder side. "
                   "See README for specific formatting instructions.")
    group.add('--pre_word_vecs_exp', '-pre_word_vecs_exp',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the context_encoder side. "
                   "See README for specific formatting instructions.")
    group.add('--pre_word_vecs_oth', '-pre_word_vecs_oth',
              help="If a valid path is specified, then this will load "
                   "pretrained word embeddings on the context_encoder side. "
                   "See README for specific formatting instructions.")
...
def translate_opts(parser):
    ...
    group.add('--des', '-des',
              help="Description sequence to decode (one line per sequence)")
    group.add('--rep', '-rep',
              help="Reproduce sequence to decode (one line per sequence)")
    group.add('--exp', '-exp',
              help="Expected behavior sequence to decode (one line per sequence)")
    group.add('--oth', '-oth',
              help="Other sequence to decode (one line per sequence)")
    group.add('--tgt', '-tgt',
