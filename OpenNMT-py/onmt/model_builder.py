...
def build_context_encoder(opt, embeddings):
    enc_type = "bcrnn"
    return str2enc[enc_type].from_opt(opt, embeddings)
...
def build_context_encoder_with_embeddings(model_opt, field, share_embeddings=False, src_emb=None):
    # Build context encoder.
    context_emb = build_embeddings(model_opt, field)
    if share_embeddings:
        context_emb.word_lut.weight = src_emb.word_lut.weight
    encoder = build_context_encoder(model_opt, context_emb)

    return encoder, context_emb
...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.model_task == ModelTask.SEQ2SEQ:
        ...
        encoder, src_emb = build_encoder_with_embeddings(model_opt, fields)
        des_encoder, des_emb = build_context_encoder_with_embeddings(model_opt, fields['des'],
            share_embeddings=model_opt.share_embeddings, src_emb=src_emb)
        rep_encoder, rep_emb = build_context_encoder_with_embeddings(model_opt, fields['rep'],
            share_embeddings=model_opt.share_embeddings, src_emb=src_emb)
        exp_encoder, exp_emb = build_context_encoder_with_embeddings(model_opt, fields['exp'],
            share_embeddings=model_opt.share_embeddings, src_emb=src_emb)
        oth_encoder, oth_emb = build_context_encoder_with_embeddings(model_opt, fields['oth'],
            share_embeddings=model_opt.share_embeddings, src_emb=src_emb)
        decoder, _ = build_decoder_with_embeddings(
            model_opt,
            fields,
        ...
        return onmt.models.ContextModel(src_encoder=encoder, decoder=decoder, des_encoder=des_encoder, 
                                        rep_encoder=rep_encoder, exp_encoder=exp_encoder, 
                                        oth_encoder=oth_encoder, context_ratio = model_opt.context_ratio)
    ...
    if checkpoint is None or model_opt.update_vocab:
        ...
        if model_opt.model_task == ModelTask.SEQ2SEQ:
            ...
            model.src_encoder.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc)
            model.des_encoder.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc)
            model.rep_encoder.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc)
            model.exp_encoder.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc)
            model.oth_encoder.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc)
